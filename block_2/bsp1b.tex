\solution{
	...
	Then by constructing of the equivalences for the SFOs, we get the
	transformation by translating the labeled subformulas of the above tree into
	CNF (\textit{local constraints}):
	...
	Translating the formula $\psi $ into a definitational form, where each
	logical gate is assinged with a label $l_{i}$, we get $\hat{\delta}(\psi )$
	by conjuncting all local constraints (clauses) of table (\ref%
	{tab:translation_cnf}), such that:

	\begin{equation*}
	\hat{\delta}(\psi )=l_{4}\AND(\lnot l_{1}\OR\lnot a\OR c)\ANDdots l_{4}\OR%
	\lnot l_{3}.
	\end{equation*}

	Finally, the definitational formula $\hat{\delta}(\psi )$ is \textit{%
	sat-equivalent} to the original formula $\psi $.
