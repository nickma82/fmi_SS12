...
%TCIDATA{LastRevised=Sunday, April 01, 2012 04:24:19}
...
G_{\pi }=(V,E_{\pi })(=\langle V_{i}\rangle ?)$ with $E_{\pi }=\{(\pi
(v),v)\mid v\in V$ and $\pi (v)\neq $ \textsc{Null}$\}$, forms a \textit{%
depth-first forest} which is composed of serveral \textit{depth-first trees}%
. The edges in $E_{\pi }$ are also called \textit{tree edges}.
...
%	\Ensure A $k$-coloring of $G_{T}$.
%	\newline
%
%	\Procedure{\textsc{Dfs}}{$G_{T}$}
%		\ForAll{vertex $u \in V[G_{T}]$}
%			%\State $c[u] \gets i \in \{1,\ldots, k\} \text{ s.t. } c[u] \neq c[\pi[u]]$
%			\State $c[u] \gets 1$ \Comment{paint initially all vertices with color $1$.}
...
%		%\State $c[u] \gets 0$ \Comment{vertex $u$ with color $i \in \{1,\ldots, k\}$ has been discovered.}
%		\State $c[u] \gets 0$ \Comment{vertex $u$ with color $1$ has been discovered.}
%		\State $d[u] \gets time \gets time + 1$ \Comment{update the discovering time.}
%		\ForAll{$v \in adj[u]$} \Comment{explore edge $(u,v)$.}
%		   %\If {$c[v] = i \in \{1,\ldots,k\} \text{ s.t. } c[v] \neq c[\pi[v]]$}
%		   \If {$c[v] = 1$}
%			   \State $\pi[v] \gets u$
...
%		 \Statex\Comment{paint vertex $u$ with color $i \in \{1,\ldots,k\}$; $\Rightarrow$ finished.}
%		 \State $f[u] \gets time \gets time + 1$ \Comment{update the finishing time.}
%	\EndProcedure	
%\end{algorithmic}
...
The algortihm works as follows. Lines 2--4 paint all vertices with color $1$
and initialize their predecessor (parent node) to \textsc{Null}. Line 5
initialize the global time counter with $0$.
